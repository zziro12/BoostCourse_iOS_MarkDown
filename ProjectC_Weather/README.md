...
선택된 국가이름이 두번쨰 뷰의 내비게이션 아이템의 타이틀, state를 이용하여 알맞은 이미지 넣기, 섭씨화씨로 변환해서 띄우기(소수첫번째자리 까지만 허용), 레이블의 텍스트색깔 조건에따라 조절, CodingKeys를 이용하여 더 매끄럽게 설정.  
세그를 통해 셀누르면 세번째 화면으로 이동하게 설정, 데이터 넘겨주기, 세번째화면 레이아웃 만들기, 받은데이터를 가지고 세번째화면에 적용하기(타이틀, 이미지, 날씨, 섭씨화씨, 강수확률, 텍스트색깔설정을 위한 데이터)
>> 가능하면 MVVM패턴적용.. 까지는 못한체 일단 완성,     동작구현 잘되는지 전체적으로 점검,  코딩스타일 이상한건없는지 확인, 주석달기
