...
첫번째 제출에서 해결하지 못한부분에 대해 질문하고, 두번째 구현에서 이제 싱글톤을 이용한 정보저장은 다했고 버튼을 눌렀을때 정보를 지우는것과 화면전환시 정보를 오브젝트들에 넣어놓는 작업만 완성하고, 내 핸드폰으로 돌려본후(못함..), 코드문법잘못된거 없나 확인후 제출하면 될듯하다!!       

두번째 제출 - 첫번째와 차이점(스택뷰 사용, 버튼활성화 기능구현 보다 깔끔하게 잘됨, 코드의 일관성, 싱글톤을 이용한 정보저장&삭제).   
- 패스 받음:), 그보다 화면전환 부분에 대해서 부족한부분을 고쳐보았으면 좋겠다고 조언을 해주셨고, 버튼활성화에 대한 고민부분도 조언을 통해 해결하는 작업이 더욱 뿌듯했던것 같다. 리뷰어분들께 너무 감사드린다.!   
